...
export default function HomeEventItem({ post, order, largePost }) {
    ...
    >
      ...
      >
        ...
        <Link component={RouterLink} href={paths.career.post} color="inherit">
          <TextMaxLine variant={largePost ? "h3" : "h6"}>
            {post.title}
          </TextMaxLine>
        </Link>

        {largePost && (
          <Typography sx={{ opacity: 0.48 }}>{post.description}</Typography>
        )}
      </Stack>
    </Stack>
  );
